...
class Scheduler(nn.Module):
    def __init__(self, N, device, grad_indexes, use_deepsets=True):
    ...
    def compute_loss(self, batch_support, batch_query, model):
        ...
        for (data_s, data_q) in zip(batch_support, batch_query):
            loss_meta_query = model.module.forward_meta(data_s, data_q)
    ...
    def get_weight(self, batch_support, batch_query, model, pt):
        ...
        for (data_s, data_q) in zip(batch_support, batch_query):
            ...
            loss_support,_ = model.module.forward(data_s)
            loss_query,_ = model.module.forward(data_q)

            fast_weights = OrderedDict(model.module.named_parameters())
            # gradients = torch.autograd.grad(loss_support, fast_weights.values(), create_graph=False, allow_unused=True)
            # for i, grad in enumerate(gradients):
            #     if grad is None:
            #         print(f"Warning: Tensor in fast_weights[{i}] was not used in the computation of loss_support.")
            #     else:
            #         print(f"Tensor in fast_weights[{i}] was used, and its gradient is non-zero.")
            ...
            # task_grad_support = torch.autograd.grad(loss_support, create_graph=False)
            # task_grad_query = torch.autograd.grad(loss_query, create_graph=False)
            task_grad = []
            ...
            for i in range(len(task_grad)):
                task_grad_cos.append(
                    self.cosine(task_grad_support[i].flatten().unsqueeze(0), task_grad_query[i].flatten().unsqueeze(0))[
                        0])
        ...
        task_layer_inputs = [torch.stack(input_embedding_norm).to(self.device), torch.stack(input_embedding_cos).to(self.device)]

        weight = self.forward(task_losses, task_layer_inputs,
                              torch.tensor([pt]).long().repeat(len(task_losses)).to(self.device))
